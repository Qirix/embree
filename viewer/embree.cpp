...
namespace embree
{
  ...
  static void parseProfileRenderer(Ref<ParseStream> cin, const FileName& path)
  {
    g_renderer = g_device->rtNewRenderer("profile");
    if (g_depth >= 0) g_device->rtSetInt1(g_renderer, "maxDepth", g_depth);
    ...
    while (cin->peek() != "}") {
      ...
      cin->force("=");
      if (tag == "depth") g_device->rtSetInt1(g_renderer, "maxDepth", cin->getInt());
      else std::cout << "unknown tag \"" << tag << "\" in profile renderer parsing" << std::endl;
    }
    cin->drop();
  ...
  static void parseSkinRenderer(Ref<ParseStream> cin, const FileName& path)
  {

    g_renderer = g_device->rtNewRenderer("skin");
    g_device->rtSetInt1(g_renderer, "sampler.spp", g_spp);
    if (g_depth >= 0) g_device->rtSetInt1(g_renderer, "maxDepth",  g_depth);

    if (cin->peek() != "{") goto finish;  cin->drop();  while (cin->peek() != "}") {

      std::string tag = cin->getString();  cin->force("=");
      if      (tag == "blurStrength"   ) g_device->rtSetFloat1(g_renderer, "skinBlurStrength", cin->getFloat());
      else if (tag == "blurDepthScale" ) g_device->rtSetFloat1(g_renderer, "skinBlurScale"   , cin->getFloat());
      else if (tag == "depth"          ) g_device->rtSetInt1  (g_renderer, "maxDepth"        , cin->getInt()  );
      else if (tag == "spp"            ) g_device->rtSetInt1  (g_renderer, "sampler.spp"     , cin->getInt()  );
      else if (tag == "minContribution") g_device->rtSetFloat1(g_renderer, "minContribution" , cin->getFloat());
      else if (tag == "transmission"   ) g_device->rtSetFloat1(g_renderer, "skinTransmission", cin->getFloat());
      else std::cout << "unknown tag \"" << tag << "\" in skin renderer parsing" << std::endl;
