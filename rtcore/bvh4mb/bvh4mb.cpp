...
namespace embree
{
  ...
  Ref<RefCount> BVH4MB::query(const char* interface)
  {
    if (trity.name == "triangle4i") {
      if (!strcmp(interface,Intersector::name)) {
      ...
      throw std::runtime_error("unknown triangle intersector interface \""+std::string(interface)+"\"");
    }
